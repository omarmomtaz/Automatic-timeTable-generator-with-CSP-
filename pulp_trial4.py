...
import pulp
import pandas as pd
from datetime import datetime, time as dt_time  # Rename time to dt_time to avoid conflict
...
for session in sessions:
    ...
    for ts in data['timeslots']:
        for room in [r for r in data['rooms'] if r.get('Type', '') in allowed_room_types and int(r.get('Capacity', '0')) >= int(session.get('student_count', '0'))]:
...
start_time = time.time()  # Using time module for timing
...
if pulp.LpStatus[prob.status] == 'Optimal':
    ...
    # Define time slots based on 9:00 AM to 3:45 PM range
    time_slots = [
        ('9:00-9:45', '9:45-10:30', '10:45-11:30', '11:30-12:15'),
        ('12:30-1:15', '1:15-2:00', '2:15-3:00', '3:00-3:45')
    ]
    days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday']

    # Helper function to convert time to minutes since midnight
    def time_to_minutes(t):
        if isinstance(t, dt_time):  # Using dt_time instead of time
            return t.hour * 60 + t.minute
        elif isinstance(t, str):
            try:
                return datetime.strptime(t, '%H:%M').hour * 60 + datetime.strptime(t, '%H:%M').minute
            except ValueError:
                print(f"Warning: Invalid time string format for value: {t}, defaulting to 0")
                return 0
        print(f"Warning: Unexpected type for time value: {type(t)}, value: {t}, defaulting to 0")
        return 0
    ...
    for sess_id, (time_id, room_id, instr_id) in solution.items():
        ...
        if sess:
            ...
            year = sess.get('required_year', 1)
            try:
                section = int(sess.get('section_id', '1'))
            except ValueError:
                print(f"Warning: Invalid section_id for session {sess_id}, defaulting to 1")
                section = 1
            group = (section - 1) // 3 + 1  # Group 1: sections 1-3, Group 2: sections 4-6, Group 3: sections 7-9
            start_time = time.get('StartTime', '00:00')
            start_minutes = time_to_minutes(start_time)
            day = time.get('Day', 'Unknown')
            slot_idx = next((i for i, slots in enumerate(time_slots) if any(start_minutes >= time_to_minutes(datetime.strptime(s.split('-')[0], '%H:%M')) and start_minutes < time_to_minutes(datetime.strptime(s.split('-')[1], '%H:%M')) for s in slots)), None)
            if slot_idx is None:
                print(f"Warning: No valid slot found for session {sess_id} with start time {start_time}")
                slot_idx = 0
            ...
            details = f"{sess.get('course_id', 'Unknown')} ({sess.get('type', 'Unknown')}) by {instr.get('name', 'Unknown')} - {room_id}"
    ...
    # Create multi-index DataFrame with days and time slots as rows, and year-group-section as columns
    df_timetable = pd.DataFrame(timetable_data)
    columns = pd.MultiIndex.from_product(
        [[1, 2, 3, 4],  # Years
         [1, 2, 3],     # Groups (up to 3 groups per year)
         [1, 2, 3]],    # Sections (3 sections per group)
        names=['Year', 'Group', 'Section']
    )
    index = pd.MultiIndex.from_product([days, [slot for sublist in time_slots for slot in sublist]], names=['Day', 'TimeSlot'])
    df_pivot = pd.DataFrame(index=index, columns=columns)
    ...
    for _, row in df_timetable.iterrows():
        ...
        section = row['Section']
        details = row['Details']
        if (day, timeslot) in df_pivot.index and (year, group, section) in df_pivot.columns:
            df_pivot.at[(day, timeslot), (year, group, section)] = details
    ...
    try:
        ...
        print(f"Warning: Failed to save to Excel: {e}. Please ensure openpyxl or xlsxwriter is installed.")
